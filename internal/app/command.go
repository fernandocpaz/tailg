...
func NewCommand(ctx context.Context, stdin io.Reader, stdout, stderr io.Writer) *cobra.Command {
	...
	flags.BoolVar(&options.HideProbes, "hide-probes", false, "hide health, ready, and live probe traffic")
	flags.BoolVar(&options.NoDefaultExclude, "no-default-exclude", false, "legacy compatibility; probe traffic is shown by default")
	_ = flags.MarkHidden("no-default-exclude")
